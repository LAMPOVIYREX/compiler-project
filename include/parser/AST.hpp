...
enum class TypeKind {
    ...
    STRING,
    STRUCT,
    VOID,      // Только для функций (ничего не возвращает)
    ARRAY,
    ERROR      
};
...
struct Type {
    ...
    // Конструкторы
    Type() : kind(TypeKind::VOID), elementType(nullptr), arraySize(0) {}
    Type(TypeKind k) : kind(k), elementType(nullptr), arraySize(0) {}
    Type(const std::string& name) : kind(TypeKind::STRUCT), structName(name), elementType(nullptr), arraySize(0) {}
    
    // Конструктор для массива
    Type(Type* elemType, int size = 0) 
        : kind(TypeKind::ARRAY), elementType(elemType), arraySize(size) {}
    ...
    Type(const Type& other) 
        : kind(other.kind), structName(other.structName), arraySize(other.arraySize) {
        if (other.elementType) {
            elementType = new Type(*other.elementType);
        } else {
            elementType = nullptr;
        }
    }
    ...
    Type(Type&& other) noexcept
        : kind(other.kind), structName(std::move(other.structName)), 
    ...
    bool isDynamicArray() const { return kind == TypeKind::ARRAY && arraySize == 0; }
