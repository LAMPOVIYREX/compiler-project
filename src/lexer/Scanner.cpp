...
std::vector<Token> Scanner::scanTokens() {
    ...
    while (!isAtEnd() && !errorReporter.hasFatalError()) {
        ...
        // ИЗМЕНЕНО: добавляем ВСЕ токены, включая ERROR
...
Token Scanner::scanNextToken() {
    ...
    switch (c) {
        ...
        case '&':
            if (peek() == '&') {
                advance();
                return makeToken(TokenType::AMP_AMP);
            }
            errorReporter.reportLexical(line, getColumn() - 1, "Unexpected character", c);
...
Token Scanner::scanNumber() {
    ...
    if (peek() == 'e' || peek() == 'E') {
        ...
        if (!isDigit(peek())) {
            errorReporter.reportInvalidFloat(line, getColumn(), 
    ...
    // После парсинга числа, перед возвратом токена
    std::string numberStr = source.substr(numberStart, current - numberStart);

    // Проверяем, есть ли нецифровые символы после числа
    if (isAlpha(peek())) {
    ...
    try {
        if (isFloat) {
            ...
            if (value < INT_MIN || value > INT_MAX) {
                ...
                return Token(TokenType::ERROR, numberStr, line, getColumn() - numberStr.length());
            }
