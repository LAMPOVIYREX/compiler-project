...
ASTDotGenerator::ASTDotGenerator(std::ostream& output, bool showTypes)
    : output(output), nodeCounter(0), showTypes(showTypes) {
    result << "digraph AST {\n";
...
std::string ASTDotGenerator::getTypeLabel(const Type& type) {
    if (type.isError()) {
        return "error";
    }
    if (type.kind == TypeKind::ARRAY && type.elementType) {
        return "array<" + type.elementType->toString() + ">";
    }
    return type.toString();
}

std::string ASTDotGenerator::getNodeColor(ASTNode& node) {
    // Проверяем, является ли узел выражением с типом ERROR
    if (showTypes) {
        auto it = nodeTypes.find(&node);
        if (it != nodeTypes.end() && it->second.isError()) {
            return "lightcoral";  
        }
    }
    
    // Стандартные цвета по типу узла
    ...
    if (dynamic_cast<MemberAccessExprNode*>(&node)) return "lightyellow";
...
void ASTDotGenerator::visit(LiteralExprNode& node) {
    ...
    // Добавляем тип на основе значения литерала
    if (showTypes) {
...
void ASTDotGenerator::visit(IdentifierExprNode& node) {
    ...
    int nodeId = createNode(label, "box", getNodeColor(node));
    if (!parentStack.empty()) {
        connectNodes(parentStack.top(), nodeId);
    }
}
...
void ASTDotGenerator::visit(CallExprNode& node) {
    ...
    if (showTypes) {
        // Для factorial возвращает int
        if (node.callee == "factorial") {
            label += "\\n-> int";
        } else {
            label += "\\n-> ?";
        }
...
void ASTDotGenerator::visit(ReturnStmtNode& node) {
    std::string label = "ReturnStmt";
    
    if (showTypes && node.value && nodeTypes.find(node.value.get()) != nodeTypes.end()) {
        label += "\\n[" + getTypeLabel(nodeTypes[node.value.get()]) + "]";
    }
