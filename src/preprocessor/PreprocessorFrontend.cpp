...
std::string PreprocessorFrontend::readFile(const std::string& filename) {
    ...
    if (!file.is_open()) {
        // ИСПРАВЛЕНО: используем reportGeneralError вместо reportLexical
        errorReporter.reportGeneralError(0, 0, "Cannot open file: " + filename);
        return "";
    }
...
std::string PreprocessorFrontend::preprocessFile(const std::string& filename) {
    ...
    for (const auto& error : preprocessor->getErrors()) {
        ...
        if (linePos != std::string::npos) {
            ...
            // ИСПРАВЛЕНО: используем reportGeneralError вместо reportLexical
            errorReporter.reportGeneralError(line, 1, error);
        } else {
            errorReporter.reportGeneralError(0, 0, error);
        }
    }
